export const IMAGES = [
  {
    src: '/screenshot1.png',
    url: 'https://github.com/tommo593/hagl-nextjs',
    title: 'Hagl - an online estate agency',
  },
  {
    src: '/screenshot2.png',
    url: 'https://job-satisfaction-ts.vercel.app/',
    title: 'Job Satisfaction Survey',
  },
  {
    src: '/screenshot3.png',
    url: 'https://github.com/tommo593/portfolio-vite',
    title: "This site's code",
  },
];

export const DESKTOP_IMAGES = [
  {
    id: '1.',
    src: '/cbss.png',
    url: 'https://github.com/tommo593/portfolio-vite',
    title: 't.ohanlon',
    placeholder: 'The code for this site',
    description: 'React, Vite, Tailwind',
  },
  {
    id: '2.',
    src: '/jsss.png',
    url: 'https://job-satisfaction-ts.vercel.app/',
    title: 'Fulfilld',
    placeholder: 'Job Satisfaction Survey',
    description: 'React, Vite, Tailwind, chart.js',
  },
  {
    id: '3.',
    src: '/hss.png',
    url: 'https://github.com/tommo593/hagl-nextjs',
    title: 'hagl',
    placeholder: 'An online estate agency',
    description: 'React, Next.js, Tailwind',
  },
  {
    id: '4.',
    src: '/tass.png',
    url: 'https://github.com/tommo593/travel-app/tree/main',
    title: 'Travel app',
    placeholder: 'Travel app',
    description: 'React, Next.js, Tailwind',
  },
  {
    id: '5.',
    src: '/pern_screenshot.png',
    url: 'https://github.com/tommo593/pern-todo',
    title: 'Todo List',
    placeholder: 'PERN CRUD App',
    description: 'React, Express, Node, PostgreSQL',
  },
  {
    id: '6.',
    src: '/chrome_ext_screenshot.png',
    url: 'https://github.com/tommo593/concert-chrome-ext/tree/main',
    title: 'Local Concert Finder',
    placeholder: 'A Chrome Extension that keeps track of concerts in your area',
    description: 'HTML, CSS, JavaScript',
  },
  {
    id: '7.',
    src: '/poke_api_screenshot.png',
    url: 'https://github.com/tommo593/poke_api',
    title: 'Pokémon Shiny Tracker',
    placeholder: "Keep track of what Pokemon you've caught and whether they're shiny or not",
    description: 'Vite, PostgreSQL, GraphQL, Express',
  },
  {
    id: '8.',
    src: '/han_rec_screenshot.png',
    url: 'https://github.com/tommo593/han-rec',
    title: 'Find-a-Noob',
    placeholder: 'Showcase site built for members of my Hire-a-Noob Discord server',
    description: 'Vite, Tailwind, Supabase',
  },
];
